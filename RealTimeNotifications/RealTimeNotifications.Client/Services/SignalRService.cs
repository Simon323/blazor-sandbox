...
public class SignalRService : IAsyncDisposable
{
	...
	public async Task StartConnection()
	{
		...
		_hubConnection = new HubConnectionBuilder()
			.WithUrl(_navigationManager.ToAbsoluteUri("/retrohub"))
			.Build();

		_hubConnection.On<RetrospectiveItem>(nameof(IRetroHub.ReceiveRetroItem), item => OnItemReceived?.Invoke(item));
		_hubConnection.On<int>(nameof(IRetroHub.UpdateClientsCount), count => OnUpdateClientsCount?.Invoke(count));
